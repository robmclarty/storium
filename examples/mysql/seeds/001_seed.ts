...
export default defineSeed(async (db) => {
  ...
  await posts.create({ title: 'MySQL Tips', body: 'JSON functions, stored procedures...', status: 'published', author_id: users.ref({ email: 'bob@example.com' }), tags: ['tutorial', 'mysql'], metadata: { featured: true } })
})
