...
export default defineSeed(async (db) => {
  ...
  await posts.create({
    ...
    tags: ['tutorial', 'storium'],
    metadata: { featured: true },
  })

  await posts.create({
    title: 'Advanced Queries',
    body: 'Deep dive into custom queries...',
    status: 'published',
    author_id: users.ref({ email: 'alice@example.com' }),
  ...
  await posts.create({
    ...
    status: 'draft',
    author_id: users.ref({ email: 'bob@example.com' }),
    tags: ['draft'],
  })

  await posts.create({
    title: 'SQLite Tips',
    body: 'JSON functions, lightweight storage...',
    status: 'published',
    author_id: users.ref({ email: 'bob@example.com' }),
    tags: ['tutorial', 'sqlite'],
    metadata: { featured: true },
  })
})
