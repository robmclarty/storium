...
`driverOptions`, which is spread into the pool/client constructor. You keep
...
`dbCredentials` (a `connectionString` / `uri` inside `driverOptions` is a
`ConfigError`, not a silent override), and `pool.min` / `pool.max` win over the
same keys given in `driverOptions`. The option is untyped because the drivers
are optional peers; add `satisfies Partial<PoolConfig>` at the call site if you
