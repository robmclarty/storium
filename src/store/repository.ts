...
const buildDefaultCrud = (
  ...
  /**
   * For MySQL (no RETURNING): resolve $defaultFn for PK columns so that the
   * generated value is available for the follow-up SELECT. Mutates `prepared`.
   */
  const fillPkDefaults = (prepared: Record<string, any>) => {
  ...
  const create = async (input: Record<string, any>, opts?: PrepOptions) => {
    ...
    try {
      ...
      // Resolve $defaultFn values before insert so we have PK for the SELECT.
      fillPkDefaults(prepared)
  ...
  const destroyAll = async (filters: Record<string, any>, opts?: PrepOptions) => {
    ...
    const result: any = await getDb(opts)
      .delete(table)
      .where(buildWhere(filters, opts))

    // PG: result.rowCount, SQLite: result.changes, MySQL: [ResultSetHeader, FieldPacket[]]
    return result.rowCount ?? result.changes ?? result[0]?.affectedRows ?? 0
  }
  ...
  const createMany = async (inputs: Record<string, any>[], opts?: PrepOptions) => {
    ...
    // MySQL: no RETURNING — resolve $defaultFn for PKs, insert, then SELECT back.
    for (const row of preparedRows) fillPkDefaults(row)
